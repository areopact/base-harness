...
| Skills | 15 skill folders ship as source under `harness/skills/`, all `spec-only`: core (7) and maintain (4) are selected by default and materialized per runtime (every catalog reads `11 selected of 15 available`); decks (2), memory (1), and delegation (1) stay on disk until selected. The doctor skill has been invoked live on all three runtimes; its `metadata:` block loaded without a loader warning on Claude Code and OpenCode, and the generated Codex wrapper (name and description only) loaded on Codex | `docs/PACKS.md` |
