...
|---|---|---|---|
| `structure.json` | registry | Host facts: the five memory lanes, git mode, outbound globs, write-deny globs and exceptions, brain local path and tracking, tier defaults, delegation flag, selection scope, contract mode (`rendered` or `host-owned`), and adopted-host record (`host.adopted`, `host.roots`, `host.harness_owned`, `host.profile` (`solo` or `team`), `host.verify_command` (the host's own package-manager or make verify command, or null)) | every kernel file through `harness_registry.load_structure()` or `hook_io.load_structure()`; `init.py`, `adopt.py`, `export.py`, the doctors, `contract_files.py`, `lint.py` (L6), `deidentify_lint.py` (D3, D6) |
