...
Template: this file is a shape, not content. Edit it once the module is scaffolded; the harness owns no memory content.
...
Scaffold the module with `python harness/tools/init.py --brain`. Lane folders are created on first write; a fresh clone carries only this page and `brain/shared/README.md` (the local guide sits inside the ignored local lane); `python harness/tools/init.py --brain` places the rest.
