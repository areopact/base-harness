...
| research | quick cited answer, `deep` and `ingest` modes | rewritten; no machine-specific tier filter; declares `distribution: runtime-provided` against `web-search` and `delegated-execution`; when delegated execution is absent, deep mode runs its stages sequentially in the main loop and says so | spec-only |
