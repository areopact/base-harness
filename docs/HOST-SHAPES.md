...
- **`python -m pytest harness`**: adopt also lands the template's own test suite under `harness/`. A guard or frontmatter fixture's expected verdict is written against the template's shipped structure (main-only git mode, populated lanes), not whatever the host's `structure.json` says, so the affected fixture runners pin `HARNESS_STRUCTURE_FILE` (see `harness/hooks/README.md`) to the template's fixed default for their own test process only; production hooks never set it. A handful of other tests assert this checkout's own state directly (the template's shipped `structure.json` equals its defaults, this checkout's `README.md` and `docs/` carry the current skill-catalog count, `adopt.py` runs cleanly against this checkout as its own source template) and are not meaningful once this checkout is itself an adopted host, so they skip with a stated reason keyed on `structure.json`'s `host.adopted` field. With those two mechanisms, `python -m pytest harness` from the host root is a valid, deterministic host check: it reports genuine host-side regressions, not artifacts of the host's structure or its already-adopted state.
- **The executable bit**: an apply run stages `git update-index --add --chmod=+x` for every landed `harness/*.sh` file and `.githooks/pre-commit`, the same scope `L18` checks. The adopter's own commit carries those staged modes forward with the rest of the diff, and on Windows this is the only way the bit reaches history at all: `core.filemode` is `false` there, so the working-tree copy has no executable bit for git to read at commit time, and a commit made without the staged mode lands the scripts at `100644`. Run `python harness/tools/lint.py --strict` again after that first commit, not only before it: `L18` reads the git index, so it cannot see a missing bit until the commit that would carry (or miss) it actually exists.
