...
In branches mode the local lane lives outside the repository, and the operator profile resolves per git user. The structure validator accepts that external path (it is not restricted to repository-relative `local_path` values); `adopt.py` and `init.py --profile team` both write it, and `load_identity` reads the identity lane paths verbatim, external or not. The path is `~/.harness-local/<name>`, where `<name>` is the basename of the target's git `origin` remote (falling back to the target folder's own name when there is no usable origin), so the path names the repository rather than one adopter's local checkout folder. A branches-mode host therefore keeps the local lane untracked outside each contributor's own clone, one directory per repository under the contributor's home. `brain/README.md` and `brain/local/README.md` carry the detail.
...
A solo repository that gains collaborators runs `python harness/tools/init.py --profile team --yes`. An untouched solo clone (still at the shipped defaults for `git.mode`, `brain.local_path`, and the lanes) gets the whole team preset from that one command: `host.profile` set to `team`, `git.mode` to `branches`, and `brain.local_path` to the external `~/.harness-local/<name>` path. A host that already configured any of those gets `host.profile` changed only, with the command printing what it kept; answering the interview (no `--yes`) is what changes them instead. Either way, change the remaining things by hand, in order: set `selection_scope` to `user`; review every page under `brain/shared/` against the new audience and raise labels before the first collaborator clones; run `python harness/tools/export.py --tier internal --out <scratch dir>` and read the result as a collaborator would. Then run `python harness/tools/lint.py --strict` and the three doctors. The full procedure with its rationale is in `brain/README.md`.
